...
export default function ProductCard({ product }) {
  ...
  return (
    <div
      className="bg-white group border border-black/5 hover:border-black/20 transition-all duration-300 cursor-pointer"
      onClick={handleClick}
    >
      {/* Image */}
      <div className="relative overflow-hidden bg-offwhite">
        ...
        {isUnavailable && (
          <div className="absolute inset-0 bg-white/70 flex items-center justify-center">
            <span className="text-black/40 text-xs tracking-[0.3em] uppercase font-medium">
              Sold Out
            </span>
          </div>
        )}
      ...
      <div className="p-4 border-t border-black/5">
        ...
        {product.description && (
          <p className="text-black/40 text-xs mb-3 line-clamp-2 leading-relaxed font-light">
            {product.description}
          </p>
        )}
        ...
        {isUnavailable ? (
          <button
            disabled
            className="w-full py-2.5 text-xs tracking-widest uppercase font-medium text-black/30 border border-black/10 cursor-not-allowed"
          >
            Sold Out
          </button>
          ...
          <button
            onClick={handleAdd}
            className="w-full py-2.5 bg-black hover:bg-gold hover:text-black text-white text-xs tracking-widest uppercase font-medium transition-colors"
          >
            Select Options
          </button>
